  ...
  describe ItemParser do
    ...
    describe '.parse' do
      ...
      it 'creates the correct Items' do
        ...
        expect(items[0].unit_price).to eq BigDecimal.new(8000, 4) / 100
        expect(items[0].created_at).to eq Time.parse('2016-01-11 12:17:26 UTC')
        expect(items[0].updated_at).to eq Time.parse('1988-06-26 02:51:02 UTC')
        ...
        expect(items[1].unit_price).to eq BigDecimal.new(100000, 4) / 100
        expect(items[1].created_at).to eq Time.parse('2016-01-11 17:41:20 UTC')
        expect(items[1].updated_at).to eq Time.parse('1974-11-29 08:58:13 UTC')
        ...
        expect(items[2].unit_price).to eq BigDecimal.new(60000, 4) / 100
        expect(items[2].created_at).to eq Time.parse('2016-01-11 11:30:35 UTC')
        expect(items[2].updated_at).to eq Time.parse('1986-12-21 16:16:32 UTC')
      end
